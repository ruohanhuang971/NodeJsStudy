                ...
                {/* display filtered books*/}
                {
                    news.length > 0 && news.map((item, index) => {
                        return <SwiperSlide key={index}>
                            <div className='flex sm:flex-row sm:justify-between flex-col items-center gap-12'>
                                {/*content*/}
                                <div className='py-4'>
                                    <Link to='/'>
                                        <h3 className='text-lg font-medium hover:text-blue-500 mb-4'>{item.title}</h3>
                                        <div className='w-10 h-[4px] bg-primary mb-5'></div>
                                        <p className='text-sm text-gray-600 font-secondary'>{item.description}</p>
                                    </Link>
                                </div>
                                <div className='flex-shrink-0'>
                                    <img src="" alt="" className='w-full object-cover outline-amber-400' />
                                </div>
                            </div>
                        </SwiperSlide>
                    })
